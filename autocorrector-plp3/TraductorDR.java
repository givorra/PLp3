    ...
    public class TraductorDR {
        ...
        private class Simbolo
        {
            ...
            public Simbolo(String nombre, String tipo, boolean esArray)
            {
                this.nombre     = nombre;
                this.tipo       = tipo;
        ...
        public boolean existeSimboloEnAmbito(String ambito, String simbolo)
        {
        ...
        public boolean existeSimboloGlobal(String simbolo)
        {
        ...
        public String getAmbitoSimbolo(String ambitoActual, String simbolo)
        {
            Ambito a = getAmbito(ambitoActual);
        ...
        public String S()
        {
            ...
            if(token.tipo == Token.INT || token.tipo == Token.DOUBLE || token.tipo == Token.MAIN)
            {
                ...
                traduccion += Sp().trad + "public static void main(String[] args) {\n";
    ...
    */
        ...
        public Atributos V(Atributos p_atributos)
        {
            ...
            if(token.tipo == Token.INT || token.tipo == Token.DOUBLE)
            {
                ...
                */
                addRegla(11);
                Atributos atr_tipo = Tipo();
                Atributos atr_var = Var(new Atributos("", atr_tipo.trad, atr_tipo.tipo, "","", p_atributos.prefijo));
                atributos.trad = "  " + atr_var.trad;
                //simbolos.add(new Simbolo(p_atributos.prefijo))
                atributos.trad += MV(new Atributos("", atr_tipo.trad, atr_var.tipo, "","", p_atributos.prefijo)).trad;
            }
            else
                errorSintaxis(Token.DOUBLE, Token.INT);
        ...
        public Atributos Var(Atributos p_atributos)
        {
            ...
            if(token.tipo == Token.ID)
            {
                ...
                */
                ...
                if(existeSimboloEnAmbito(p_atributos.prefijo, idlexema))
                ...
                atributos.tipo = p_atributos.tipo;
                atributos.esvariable = p_atributos.esvariable;
        ...
        public Atributos MV(Atributos p_atributos)
        {
            ...
            if(token.tipo == Token.COMA)
            {
                ...
                */
                ...
                atributos.trad = "  " + Var(new Atributos("", p_atributos.th, p_atributos.tipo,"","", p_atributos.prefijo)).trad;
                atributos.trad += MV(new Atributos("", p_atributos.th, p_atributos.tipo,"","", p_atributos.prefijo)).trad;
            }
        ...
        public Atributos Instr(Atributos p_atributos)
        {
            ...
            if(token.tipo == Token.ID)
            {
                ...
                if(!existeSimboloGlobal(token.lexema))
                    errorSemanticoIdNoDeclarado();

                String idlexema = token.lexema;
                atributos.cuerpo = "  " + getAmbitoSimbolo(p_atributos.prefijo, idlexema) + idlexema + " = ";
                emparejar(Token.ID);
                emparejar(Token.ASIG);
                //atributos.cuerpo += Expr(new Atributos("", "","","","", p_atributos.prefijo)).trad +";\n";
                Atributos atr_expr = Expr(new Atributos("", "","","","", p_atributos.prefijo));

                if(getSimbolo(p_atributos.prefijo, idlexema).tipo.equals("double"))
                {
                    if(atr_expr.esvariable) 
                    {
                        if(atr_expr.tipo.equals("double"))
                        {
                            atributos.cuerpo += atr_expr.trad + ";\n";
                        }
                        else    //atr_expr.tipo.equals("int")
                        {
                            atributos.cuerpo += "new Double(" + atr_expr.trad + ".toDouble());\n";
                        }
                    ...
                    else
                    {
                        if(atr_expr.tipo.equals("double"))
                        {
                            atributos.cuerpo += "new Double(" + atr_expr.trad + ");\n";
                        }
                        else    //atr_expr.tipo.equals("int")
                        {
                            atributos.cuerpo += "new Double((new Integer(" + atr_expr.trad + ")).toDouble());\n";
                        }
                    }
                }
                else
                {
                    if(atr_expr.esvariable)
                    {
                        atributos.cuerpo += "new Integer(" + atr_expr.trad + ".intValue()" + ");\n";
                    }
                    else
                    {
                        atributos.cuerpo += "new Integer(" + atr_expr.trad +");\n";
                    }
                }
                emparejar(Token.PYC);
            }
        ...
        public Atributos Expr(Atributos p_atributos)
        {
            ...
            {
                addRegla(19);
                Atributos atr_term = Term(new Atributos("", "","","","", p_atributos.prefijo));
                //atributos.trad = atr_term.trad;
                Atributos a = new Atributos(atr_term.trad, "",atr_term.tipo ,"","", p_atributos.prefijo);
                a.esvariable = atr_term.esvariable;
                Atributos atr_exprAux = ExprAux(a);
                atributos.tipo = atr_exprAux.tipo;
                //atributos.trad = atr_term.trad + atr_exprAux.trad;
                atributos.trad = atr_exprAux.trad;
                atributos.esvariable = atr_exprAux.esvariable;
            }
            else
        ...
        public Atributos ExprAux(Atributos p_atributos)
        {
            ...
            if(token.tipo == Token.ADDOP)
            {
                ...
                if(atr_term.tipo.equals(p_atributos.tipo))    // Si son dos factores del mismo tipo..
                {
                    if(p_atributos.esvariable)
                    {
                        //String ambito = getAmbitoSimbolo(p_atributos.prefijo, p_atributos.trad);
                        p_atributos.trad = getAmbitoSimbolo(p_atributos.prefijo, p_atributos.trad) + p_atributos.trad + "." + p_atributos.tipo + "Value()";
                    }
                    if(atr_term.esvariable)
                    {
                        //String ambito = getAmbitoSimbolo(p_atributos.prefijo, atr_term.trad);
                        atr_term.trad = getAmbitoSimbolo(p_atributos.prefijo, atr_term.trad) + atr_term.trad + "." + atr_term.tipo + "Value()";
                    }
                    a.tipo = atr_term.tipo;
                }
                else    // diferentes
                {
                    // El que sea int, hay q pasarlo a double
                    if(atr_term.tipo.equals("int"))
                    {
                       if(atr_term.esvariable)
                        {
                            atr_term.trad = atr_term.trad + ".toDouble()";
                        }
                        else
                        {
                            atr_term.trad = "(new Integer(" + atr_term.trad +")).toDouble()";
                        }
                        a.tipo = p_atributos.tipo;
                    }
                    else
                    {
                        if(p_atributos.esvariable)
                        {
                            p_atributos.trad = getAmbitoSimbolo(p_atributos.prefijo, p_atributos.trad) + p_atributos.trad + ".toDouble()";
                        }
                        else
                        {
                        ...
                        if(atr_term.esvariable)
                        {
                            atr_term.trad += ".doubleValue()";
                        }
                        a.tipo = atr_term.tipo;
                    }
                }
                a.trad = p_atributos.trad + operador + atr_term.trad;
                a.prefijo = p_atributos.prefijo;
                a.esvariable = atr_term.esvariable;
                Atributos atr_exprAux = ExprAux(a);
                atributos.tipo = atr_exprAux.tipo;
                atributos.trad = atr_exprAux.trad;
                //atributos.esvariable = atr_exprAux.esvariable;
                atributos.esvariable = false;
        ...
        public Atributos Term(Atributos p_atributos)
        {
            ...
            if(token.tipo == Token.ID || token.tipo == Token.ENTERO || token.tipo == Token.REAL)
            {
                ...
                atributos.esvariable = atr_termAux.esvariable;
            }
            else
                errorSintaxis(Token.ENTERO, Token.ID, Token.REAL);
            return atributos;
        ...
        public Atributos TermAux(Atributos p_atributos)
        {
            ...
            {
                Atributos a = new Atributos();  // Parametro para TermAux
                String operador = token.lexema;
                addRegla(23);
                ...
                if(atr_factor.tipo.equals(p_atributos.tipo))    // Si son dos factores del mismo tipo..
                {
                    ...
                    {
                        //String ambito = getAmbitoSimbolo(p_atributos.prefijo, atr_factor.trad);
                        atr_factor.trad = getAmbitoSimbolo(p_atributos.prefijo, atr_factor.trad) + atr_factor.trad + "." + atr_factor.tipo + "Value()";
                    }
                    a.tipo = atr_factor.tipo;
                }
                else    // diferentes
                {
                    // El que sea int, hay q pasarlo a double
                    if(atr_factor.tipo.equals("int"))
                    {
                       if(atr_factor.esvariable)
                        {
                            atr_factor.trad = getAmbitoSimbolo(p_atributos.prefijo, atr_factor.trad) + atr_factor.trad + ".toDouble()";
                        }
                        else
                        {
                    ...
                    else
                    {
                        if(p_atributos.esvariable)
                        {
                            p_atributos.trad = p_atributos.trad + ".toDouble()";
                        }
                        else
                        {
                            p_atributos.trad = "(new Integer(" + p_atributos.trad +").toDouble()";
                ...
                a.trad = p_atributos.trad + operador + atr_factor.trad;
                a.prefijo = p_atributos.prefijo;
                Atributos atr_termAux = TermAux(a);
                atributos.tipo = atr_termAux.tipo;
                atributos.trad = atr_termAux.trad;
            ...
            else if(token.tipo == Token.ADDOP || token.tipo == Token.PYC)
            {
                ...
                if(atributos.esvariable)
                {
                    atributos.trad = getAmbitoSimbolo(p_atributos.prefijo, p_atributos.trad) + p_atributos.trad;
                }
                else
                    atributos.trad = p_atributos.trad;
            }
